    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <View style={styles.cardContainer}>
                <View style={{ justifyContent: 'center', alignItems: 'center', alignContent: 'space-around' }}>
                    <Text style={styles.titulo}>{t('join_casting')}</Text>
                    <TouchableOpacity style={styles.boton} onPress={() => navigation.navigate('SubirContenido', { id: route.params?.announcementData?.id })}>
                        <Text style={{ color: colors.tertiary,  marginTop: '5%', fontWeight: 'bold' }}>{t('upload_video')}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => navigation.navigate('BottomTabNavigator')}>
                        <Text style={{ color: colors.tertiary, marginBottom: '5%', fontWeight: 'bold' }}>{t('upload_later')}</Text>
                    </TouchableOpacity>
                </View>
            </View>
